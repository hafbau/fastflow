...
import { FASTFLOW_CHATID, ICommonObject } from '../../../../src'
...
export class TypeORMDriver extends VectorStoreDriver {
    ...
    static similaritySearchVectorWithScore = async (
        ...
        let chatflowOr = ''
        const { [FASTFLOW_CHATID]: chatId, ...restFilters } = filter || {}
        ...
        const queryString = `
            ...
            FROM ${tableName}
            WHERE ((metadata @> $2) AND NOT (metadata ? '${FASTFLOW_CHATID}')) ${chatflowOr}
