...
import { FASTFLOW_CHATID } from '../../../../src'
...
export class PGVectorDriver extends VectorStoreDriver {
    ...
    protected async adaptInstance(instance: PGVectorStore, metadataFilters?: any): Promise<PGVectorStore> {
        const { [FASTFLOW_CHATID]: chatId, ...pgMetadataFilter } = metadataFilters || {}
        ...
        instance.pool.query = async (queryString: string, parameters: any[]) => {
            ...
            // https://github.com/FastflowAI/Fastflow/pull/3367#discussion_r1804229295
            if (chatId) {
                parameters.push({ [FASTFLOW_CHATID]: chatId })

                chatflowOr = `OR metadata @> $${parameters.length}`
            }

            if (queryString.match(whereClauseRegex)) {
                queryString = queryString.replace(whereClauseRegex, `WHERE (($1) AND NOT (metadata ? '${FASTFLOW_CHATID}')) ${chatflowOr}`)
            } else {
                const orderByClauseRegex = /ORDER BY (.*)/
                // Insert WHERE clause before ORDER BY
                queryString = queryString.replace(
                    orderByClauseRegex,
                    `WHERE (metadata @> '{}' AND NOT (metadata ? '${FASTFLOW_CHATID}')) ${chatflowOr}
                ORDER BY $1
                `
                )
            }
