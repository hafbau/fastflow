...
class Milvus_VectorStores implements INode {
    ...
    async init(nodeData: INodeData, _: string, options: ICommonObject): Promise<any> {
        ...
        if (isFileUploadEnabled && options.chatId) {
            if (milvusFilter) milvusFilter += ` OR ${FASTFLOW_CHATID} == "${options.chatId}" OR NOT EXISTS(${FASTFLOW_CHATID})`
            else milvusFilter = `${FASTFLOW_CHATID} == "${options.chatId}" OR NOT EXISTS(${FASTFLOW_CHATID})`
        }
