        ...
        this.inputs = [
            {
                ...
                name: 'user_id',
                type: 'string',
                description: 'Unique identifier for the user. Required only if "Use Fastflow Chat ID" is OFF.',
                default: 'fastflow-default-user',
                optional: true
            },
            // Added toggle to use Fastflow chat ID
            {
                label: 'Use Fastflow Chat ID',
                name: 'useFlowiseChatId',
                type: 'boolean',
                description: 'Use the Fastflow internal Chat ID as the Mem0 User ID, overriding the "User ID" field above.',
                default: false,
                optional: true
            },
            {
...
const initializeMem0 = async (nodeData: INodeData, options: ICommonObject): Promise<BaseMem0Memory> => {
    ...
    if (!useFlowiseChatId && !initialUserId) {
        throw new Error('User ID field cannot be empty when "Use Fastflow Chat ID" is OFF.')
    }
    ...
    const constructorSessionId = initialUserId || (useFlowiseChatId ? 'fastflow-chat-id-placeholder' : '')
...
class Mem0MemoryExtended extends BaseMem0Memory implements MemoryMethods {
    ...
    // Selects Mem0 user_id based on toggle state (Fastflow chat ID or input field)
    private getEffectiveUserId(overrideUserId?: string): string {
        ...
        if (this.useFlowiseChatId) {
            if (overrideUserId) {
                ...
                throw new Error('Mem0: "Use Fastflow Chat ID" is ON, but no runtime chat ID (overrideUserId) was provided.')
            }
        } else {
    ...
    async getChatMessages(
        ...
        if (!flowiseSessionId) {
            console.warn('Mem0: getChatMessages called without overrideUserId (Fastflow Session ID). Cannot fetch DB messages.')
            return []
        }
    ...
    async clearChatMessages(overrideUserId = ''): Promise<void> {
        ...
        if (flowiseSessionId) {
            ...
            console.warn('Mem0: clearChatMessages called without overrideUserId (Fastflow Session ID). Cannot clear DB messages.')
        }
    }
}
