...
export const FASTFLOW_CHATID = 'flowise_chatId'
...
const getEncryptionKeyFilePath = (): string => {
    const checkPaths = [
        ...
        path.join(__dirname, '..', '..', '..', '..', '..', 'encryption.key'),
        path.join(__dirname, '..', '..', '..', '..', '..', 'server', 'encryption.key'),
        path.join(getUserHome(), '.fastflow', 'encryption.key')
    ]
    for (const checkPath of checkPaths) {
        if (fs.existsSync(checkPath)) {
            return checkPath
...
const getEncryptionKey = async (): Promise<string> => {
    if (process.env.FASTFLOW_SECRETKEY_OVERWRITE !== undefined && process.env.FASTFLOW_SECRETKEY_OVERWRITE !== '') {
        return process.env.FASTFLOW_SECRETKEY_OVERWRITE
    }
    try {
...
const jsonEscapeCharacters = [
    { escape: '"', value: 'FASTFLOW_DOUBLE_QUOTE' },
    { escape: '\n', value: 'FASTFLOW_NEWLINE' },
    { escape: '\b', value: 'FASTFLOW_BACKSPACE' },
    { escape: '\f', value: 'FASTFLOW_FORM_FEED' },
    { escape: '\r', value: 'FASTFLOW_CARRIAGE_RETURN' },
    { escape: '\t', value: 'FASTFLOW_TAB' },
    { escape: '\\', value: 'FASTFLOW_BACKSLASH' }
]
