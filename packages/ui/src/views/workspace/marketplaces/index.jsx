...
// API
import workspaceMarketplacesApi from '@/api/workspace/marketplaces'
...
const WorkspaceMarketplace = () => {
    ...
    // Use workspace-scoped API methods
    const getAllTemplatesApi = useApi(() => workspaceMarketplacesApi.getAllMarketplaces(workspaceId))
    ...
    // Use workspace-scoped API methods
    const getAllCustomTemplatesApi = useApi(() => workspaceMarketplacesApi.getMarketplaceTemplates(workspaceId))
    ...
    useEffect(() => {
        // Fetch marketplaces data for this workspace
        getAllTemplatesApi.request()
    }, [workspaceId])
    ...
    const onDeleteCustomTemplate = async (template) => {
        ...
        if (isConfirmed) {
            try {
                // Delete template in workspace context
                const deleteResp = await workspaceMarketplacesApi.deleteTemplate(workspaceId, template.id)
                if (deleteResp.data) {
                    enqueueSnackbar({
    ...
    const goToCanvas = (selectedTemplate) => {
        if (selectedTemplate.type === 'Chatflow') {
            // Navigate to workspace-scoped chatflow
            navigate(`/workspaces/${workspaceId}/chatflows?templateId=${selectedTemplate.id}`)
        } else if (selectedTemplate.type === 'Agentflow') {
            // Navigate to workspace-scoped agentflow
            navigate(`/workspaces/${workspaceId}/agentflows?templateId=${selectedTemplate.id}`)
        }
    }
    ...
    useEffect(() => {
        ...
        if (getAllTemplatesApi.data) {
            ...
            for (let i = 0; i < getAllTemplatesApi.data.length; i += 1) {
                if (getAllTemplatesApi.data[i].usecases) {
                    marketplaceUsecases.push(...getAllTemplatesApi.data[i].usecases)
                }
            }
            setUsecases(Array.from(new Set(marketplaceUsecases)).sort())
            setEligibleUsecases(Array.from(new Set(marketplaceUsecases)).sort())
        }
    }, [getAllTemplatesApi.data])

    useEffect(() => {
        // Effect for custom templates tab
        if (getAllCustomTemplatesApi.data) {
            const customTemplatesUsecases = []
            for (let i = 0; i < getAllCustomTemplatesApi.data.length; i += 1) {
                if (getAllCustomTemplatesApi.data[i].usecases) {
                    customTemplatesUsecases.push(...getAllCustomTemplatesApi.data[i].usecases)
                }
            }
            setTemplateUsecases(Array.from(new Set(customTemplatesUsecases)).sort())
            setEligibleTemplateUsecases(Array.from(new Set(customTemplatesUsecases)).sort())
        }
    ...
    return (
        <>
            <MainCard>
                {(getAllTemplatesApi.error || error) && (
                    <Box
                        sx={{
                            p: 2
                        }}
                    >
                        <Stack sx={{ alignItems: 'center', justifyContent: 'center' }} flexDirection='column'>
                            ...
                            <div>
                                {getAllTemplatesApi.error?.response
                                    ? `Error: ${JSON.stringify(getAllTemplatesApi.error?.response?.data)}`
                                    : error?.response
                ...
                {!(getAllTemplatesApi.error || error) && (
                    <Stack alignItems='flex-start' spacing={0}>
                        <ViewHeader
                            ...
                            title={`${currentWorkspace?.name || 'Workspace'} Marketplace`}
                        >
                            <ToggleButtonGroup
                        ...
                        <TabPanel value={activeTabValue} index={0}>
                            ...
                            {!isLoading && (!getAllTemplatesApi.data || getAllTemplatesApi.data.length === 0) && (
                                <Stack sx={{ alignItems: 'center', justifyContent: 'center' }} flexDirection='column'>
                                    ...
                                    <div>No Marketplace Templates Available</div>
                                </Stack>
                            )}
                        </TabPanel>
