...
import { ICommonObject, INodeData } from 'fastflow-components'
import { convertToOpenAIFunction } from '@langchain/core/utils/function_calling'
import { v4 as uuidv4 } from 'uuid'
import { Variable } from '../../database/entities/Variable'

const SOURCE_DOCUMENTS_PREFIX = '\n\n----FASTFLOW_SOURCE_DOCUMENTS----\n\n'
...
const buildAndInitTool = async (chatflowid: string, _chatId?: string, _apiMessageId?: string) => {
    ...
    if (!toolAgentNode) {
        throw new InternalFastflowError(StatusCodes.NOT_FOUND, `Agent with tools not found in chatflow ${chatflowid}`)
    }
...
const executeAgentTool = async (
    ...
    try {
        ...
        if (!tool) {
            throw new InternalFastflowError(StatusCodes.NOT_FOUND, `Tool ${toolName} not found`)
        }
