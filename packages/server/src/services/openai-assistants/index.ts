...
import { getFileFromUpload, removeSpecificFileFromUpload } from 'fastflow-components'
...
const getAllOpenaiAssistants = async (credentialId: string): Promise<any> => {
    try {
        ...
        if (!credential) {
            throw new InternalFastflowError(StatusCodes.NOT_FOUND, `Credential ${credentialId} not found in the database!`)
        }
