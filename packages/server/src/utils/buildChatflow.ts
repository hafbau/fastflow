...
import { FASTFLOW_METRIC_COUNTERS, FASTFLOW_COUNTER_STATUS, IMetricsProvider } from '../Interface.Metrics'
...
const initEndingNode = async ({
    ...
    if (!nodeToExecute) {
        throw new InternalFastflowError(StatusCodes.NOT_FOUND, `Node not found`)
    }
...
const incrementSuccessMetricCounter = (metricsProvider: IMetricsProvider, isInternal: boolean, isAgentFlow: boolean) => {
    if (isAgentFlow) {
        metricsProvider?.incrementCounter(
            isInternal ? FASTFLOW_METRIC_COUNTERS.AGENTFLOW_PREDICTION_INTERNAL : FASTFLOW_METRIC_COUNTERS.AGENTFLOW_PREDICTION_EXTERNAL,
            { status: FASTFLOW_COUNTER_STATUS.SUCCESS }
        )
    } else {
        metricsProvider?.incrementCounter(
            isInternal ? FASTFLOW_METRIC_COUNTERS.CHATFLOW_PREDICTION_INTERNAL : FASTFLOW_METRIC_COUNTERS.CHATFLOW_PREDICTION_EXTERNAL,
            { status: FASTFLOW_COUNTER_STATUS.SUCCESS }
        )
    }
}
...
const incrementFailedMetricCounter = (metricsProvider: IMetricsProvider, isInternal: boolean, isAgentFlow: boolean) => {
    if (isAgentFlow) {
        metricsProvider?.incrementCounter(
            isInternal ? FASTFLOW_METRIC_COUNTERS.AGENTFLOW_PREDICTION_INTERNAL : FASTFLOW_METRIC_COUNTERS.AGENTFLOW_PREDICTION_EXTERNAL,
            { status: FASTFLOW_COUNTER_STATUS.FAILURE }
        )
    } else {
        metricsProvider?.incrementCounter(
            isInternal ? FASTFLOW_METRIC_COUNTERS.CHATFLOW_PREDICTION_INTERNAL : FASTFLOW_METRIC_COUNTERS.CHATFLOW_PREDICTION_EXTERNAL,
