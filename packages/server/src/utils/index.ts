...
const REDACTED_CREDENTIAL_VALUE = '_FASTFLOW_BLANK_07167752-1a71-43b1-bf8f-4f32252165db'
...
export const getVariableValue = async (
    ...
    while (startIdx < endIdx) {
        ...
        if (substr === '}}' && variableStack.length > 0 && variableStack[variableStack.length - 1].substr === '{{') {
             ...
             * FROM: hello i am ben\n\n\thow are you?
             * TO: hello i am benFASTFLOW_NEWLINEFASTFLOW_NEWLINEFASTFLOW_TABhow are you?
             */
...
export const getUserSettingsFilePath = () => {
    ...
    const checkPaths = [path.join(getUserHome(), '.fastflow', 'settings.json')]
    for (const checkPath of checkPaths) {
        if (fs.existsSync(checkPath)) {
            return checkPath
        }
...
export const getUploadPath = (): string => {
    return process.env.BLOB_STORAGE_PATH
        ...
        : path.join(getUserHome(), '.fastflow', 'uploads')
}
