import { FASTFLOW_METRIC_COUNTERS, IMetricsProvider } from '../Interface.Metrics'
import express from 'express'
import promClient, { Counter, Histogram, Registry } from 'prom-client'
import { getVersion } from 'fastflow-components'
...
export class Prometheus implements IMetricsProvider {
    ...
    async initializeCounters(): Promise<void> {
        const serviceName: string = process.env.METRICS_SERVICE_NAME || 'FastflowAI'
        this.register.setDefaultLabels({
            app: serviceName
        })

        // look at the FASTFLOW_COUNTER enum in Interface.Metrics.ts and get all values
        ...
        const enumEntries = Object.entries(FASTFLOW_METRIC_COUNTERS)
        ...
        try {
            const versionGaugeCounter = new promClient.Gauge({
                name: 'fastflow_version_info',
                help: 'Fastflow version info.',
                labelNames: ['version'],
                registers: [this.register] // Explicitly set the registry
            })
            ...
            this.counters.set('fastflow_version', versionGaugeCounter)
        } catch (error) {
            // If metric already exists, get it from the registry
            const existingMetric = this.register.getSingleMetric('fastflow_version')
            if (existingMetric) {
                this.counters.set('fastflow_version', existingMetric as promClient.Gauge<string>)
            }
        }
    ...
    public incrementCounter(counter: FASTFLOW_METRIC_COUNTERS, payload: any) {
    ...
    private registerMetrics() {
        if (process.env.METRICS_INCLUDE_NODE_METRICS !== 'false') {
            ...
            promClient.collectDefaultMetrics({
                ...
                prefix: 'fastflow_' // Add a prefix to avoid conflicts
            })
        }
