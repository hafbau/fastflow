...
export class OpenTelemetry implements IMetricsProvider {
    ...
    async initializeCounters(): Promise<void> {
        try {
            ...
            this.resource = new Resource({
                [ATTR_SERVICE_NAME]: process.env.METRICS_SERVICE_NAME || 'FastflowAI',
            ...
            const meter = this.meterProvider.getMeter('fastflow-metrics')
            ...
            try {
                ...
                if (!createdMetrics.has('fastflow_version')) {
                    const versionGuage = meter.createGauge('fastflow_version', {
                        description: 'Fastflow version'
                    })
                    ...
                    createdMetrics.set('fastflow_version', true)
                }
            } catch (error) {
    ...
    async incrementCounter(counter: FASTFLOW_METRIC_COUNTERS, payload: any): Promise<void> {
        try {
