...
const createPrediction = async (req: Request, res: Response, next: NextFunction) => {
    try {
        ...
        if (!chatflow) {
            throw new InternalFastflowError(StatusCodes.NOT_FOUND, `Chatflow ${req.params.id} not found`)
        }
        ...
        if (isDomainAllowed) {
            ...
            throw new InternalFastflowError(StatusCodes.FORBIDDEN, unauthorizedOriginError)
        }
    } catch (error) {
        next(error)
    }
