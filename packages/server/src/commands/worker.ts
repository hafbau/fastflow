...
export default class Worker extends BaseCommand {
    ...
    async run(): Promise<void> {
        logger.info('Starting Fastflow Worker...')
    ...
    async prepareData() {
        // Init database with proper async handling
    ...
    async stopProcess() {
        try {
            ...
            logger.info(`Shutting down Fastflow Prediction Worker ${this.predictionWorkerId}...`)
            await predictionWorker.close()

            const upsertWorker = queueManager.getQueue('upsert').getWorker()
            logger.info(`Shutting down Fastflow Upsertion Worker ${this.upsertionWorkerId}...`)
            await upsertWorker.close()
        } catch (error) {
            logger.error('There was an error shutting down Fastflow Worker...', error)
            await this.failExit()
        }
