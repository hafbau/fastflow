...
export abstract class BaseCommand extends Command {
    static flags = {
        FASTFLOW_USERNAME: Flags.string(),
        FASTFLOW_PASSWORD: Flags.string(),
        FASTFLOW_FILE_SIZE_LIMIT: Flags.string(),
        PORT: Flags.string(),
        ...
        DISABLE_FASTFLOW_TELEMETRY: Flags.string(),
        ...
        FASTFLOW_SECRETKEY_OVERWRITE: Flags.string(),
    ...
    protected onTerminate() {
        return async () => {
            try {
                ...
                setTimeout(async () => {
                    logger.info('Fastflow was forced to shut down after 30 secs')
                    await this.failExit()
                }, 30000)
    ...
    async init(): Promise<void> {
        ...
        // Authorization
        if (flags.FASTFLOW_USERNAME) process.env.FASTFLOW_USERNAME = flags.FASTFLOW_USERNAME
        if (flags.FASTFLOW_PASSWORD) process.env.FASTFLOW_PASSWORD = flags.FASTFLOW_PASSWORD
        ...
        // API Configuration
        if (flags.FASTFLOW_FILE_SIZE_LIMIT) process.env.FASTFLOW_FILE_SIZE_LIMIT = flags.FASTFLOW_FILE_SIZE_LIMIT
        ...
        if (flags.FASTFLOW_SECRETKEY_OVERWRITE) process.env.FASTFLOW_SECRETKEY_OVERWRITE = flags.FASTFLOW_SECRETKEY_OVERWRITE
        ...
        // Telemetry
        if (flags.DISABLE_FASTFLOW_TELEMETRY) process.env.DISABLE_FASTFLOW_TELEMETRY = flags.DISABLE_FASTFLOW_TELEMETRY
