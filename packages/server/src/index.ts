...
export class App {
    ...
    async config() {
        ...
        const fastflow_file_size_limit = process.env.FASTFLOW_FILE_SIZE_LIMIT || '50mb'
        this.app.use(express.json({ limit: fastflow_file_size_limit }))
        this.app.use(express.urlencoded({ limit: fastflow_file_size_limit, extended: true }))
        ...
        this.app.get('/api/v1/ip', (request, response) => {
            response.send({
                ip: request.ip,
                msg: 'Check returned IP address in the response. If it matches your current IP address ( which you can get by going to http://ip.nfriedly.com/ or https://api.ipify.org/ ), then the number of proxies is correct and the rate limiter should now work correctly. If not, increase the number of proxies by 1 and restart Cloud-Hosted Fastflow until the IP address matches your own. Visit https://docs.getflowstack.ai/configuration/rate-limit#cloud-hosted-rate-limit-setup-guide for more information.'
            })
        })
        ...
        const packagePath = getNodeModulesPackagePath('fastflow-ui')
