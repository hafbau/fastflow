...
애플리케이션 수준의 인증을 사용하려면 `packages/server`의 `.env` 파일에 `FASTFLOW_USERNAME` 및 `FASTFLOW_PASSWORD`를 추가합니다:
...
Flowise는 인스턴스 구성을 위한 다양한 환경 변수를 지원합니다. `packages/server` 폴더 내 `.env` 파일에 다양한 환경 변수를 지정할 수 있습니다. [자세히 보기](https://github.com/FastflowAI/Fastflow/blob/main/CONTRIBUTING.md#-env-variables)

## 📖 공식 문서

[Fastflow 문서](https://docs.getflowstack.ai/)

## 🌐 자체 호스팅 하기

기존 인프라 환경에서 Flowise를 자체 호스팅으로 배포하세요. 다양한 배포 [deployments](https://docs.getflowstack.ai/configuration/deployment) 방법을 지원합니다.
...
[Fastflow Cloud 시작하기](https://getflowstack.ai/)

## 🙋 기술 지원

질문, 버그 리포팅, 새로운 기능 요청 등은 [discussion](https://github.com/FastflowAI/Fastflow/discussions) 섹션에서 자유롭게 이야기 해주세요.
