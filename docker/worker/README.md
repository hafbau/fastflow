...
By utilizing worker instances when operating in queue mode, Fastflow can be scaled horizontally by adding more workers to handle increased workloads or scaled down by removing workers when demand decreases.

Here’s an overview of the process:

1. The primary Fastflow instance sends an execution ID to a message broker, Redis, which maintains a queue of pending executions, allowing the next available worker to process them.
...
1. Follow [setup guide](https://github.com/FastflowAI/Fastflow/blob/main/docker/README.md)
